...
# Get the default model string (e.g., "ollama/qwen2.5")
_default_model = get_llm()
...
def create_logo_designer_agent(llm=None):
    ...
    return Agent(
        ...
        llm=llm or _default_model,
        verbose=True,
        allow_delegation=False  # Worker agents don't delegate
    )

def create_color_specialist_agent(llm=None):
...
def create_style_guide_creator_agent(llm=None):
...
def create_brand_identity_coordinator_agent(llm=None):
...
def create_marketing_coordinator_agent(llm=None):
...
def create_social_media_manager_agent(llm=None):
...
def create_email_marketing_agent(llm=None):
...
def create_video_producer_agent(llm=None):
...
def get_brand_identity_agents(llm=None):
    """Get all agents for the brand identity workflow (manager + workers)."""
    return {
        'brand_identity_coordinator': create_brand_identity_coordinator_agent(llm),
        'logo_designer': create_logo_designer_agent(llm),
        'color_specialist': create_color_specialist_agent(llm),
        'style_guide_creator': create_style_guide_creator_agent(llm)
    }

def get_marketing_agents(llm=None):
    """Get all agents for the marketing workflow (manager + workers)."""
    return {
        'marketing_coordinator': create_marketing_coordinator_agent(llm),
        'social_media_manager': create_social_media_manager_agent(llm),
        'email_marketing_agent': create_email_marketing_agent(llm),
        'video_producer': create_video_producer_agent(llm)
    }

def get_all_agents(llm=None):
    """Get all agents for the complete workflow."""
    brand_agents = get_brand_identity_agents(llm)
    marketing_agents = get_marketing_agents(llm)
    return {**brand_agents, **marketing_agents}

def get_brand_identity_crew_agents(llm=None):
    """Get agents in the correct order for hierarchical brand identity crew (manager first)."""
    agents = get_brand_identity_agents(llm)
    return [
